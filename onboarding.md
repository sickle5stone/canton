...
sequenceDiagram
    ...
    App->>Vault: Read public key (transit/keys/canton-bank-treasury)
    Vault-->>App: DER-encoded public key bytes

    Note over App: fingerprint = "1220" + SHA-256(public_key_der)<br/>party_id = "bank-treasury::1220e5f6..."

    Note over Vault,Sync: PHASE 2: Generate Topology Transactions

    App->>Node: POST /v2/parties/external/generate-topology<br/>{synchronizer: "global::1220glob...",<br/>partyHint: "bank-treasury",<br/>publicKey: {format: DER, keyData: base64(pubkey)}}
    ...
    Note over App: TX2 — PartyToParticipant<br/>party == "bank-treasury::1220e5f6..."<br/>participant == Blockdaemon node ID<br/>permission == CONFIRMATION only (NOT Submission)<br/>threshold == 1

    Note over App: TX3 — PartyToKeyMapping<br/>party == "bank-treasury::1220e5f6..."<br/>signing_keys contains our public key<br/>threshold == 1
    ...
    App->>Vault: POST transit/sign/canton-bank-treasury<br/>{input: base64(multi_hash), hash_algorithm: "sha2-256",<br/>signature_algorithm: "ecdsa-p256",<br/>marshaling_algorithm: "asn1", prehashed: true}
    ...
    App->>Node: GET /v2/topology/party-to-participant?party=bank-treasury::1220e5f6...
    Node-->>App: {participant: "PAR::jpmd-bank::1220a7f3...",<br/>permission: "Confirmation", threshold: 1}

    App->>Node: GET /v2/topology/party-to-key-mapping?party=bank-treasury::1220e5f6...
    Node-->>App: {signingKeys: [{id: "1220e5f6..."}], threshold: 1}

    Note over Vault,Sync: PHASE 8: First Daml Transaction (Deposit Token Mint)

    App->>Node: POST /v2/interactive-submission/prepare<br/>{commands: [{createCommand: {<br/>templateId: "DepositToken:DepositToken",<br/>createArguments: {issuer: party, owner: party, amount: 1000000}}}],<br/>actAs: ["bank-treasury::1220e5f6..."]}
    ...
    App->>Vault: POST transit/sign/canton-bank-treasury<br/>{input: base64(tx_hash), prehashed: true}
    Vault-->>App: {signature: base64(DER signature)}

    App->>Node: POST /v2/interactive-submission/execute<br/>{preparedTransaction: base64(tx),<br/>partySignatures: {signatures: [{<br/>party: "bank-treasury::1220e5f6...",<br/>signatures: [{signature: base64(sig)}]}]}}
